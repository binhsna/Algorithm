...
// Bài toán: Nhập danh sách lien kết đơn các số nguyên. Tìm giá trị max trong danh sách
...
struct node
{
	...
	struct node* pNext; // Con trỏ liên kết giữa các node vs nhau
};
typedef struct node NODE; // Thay thế struct node th

// Khai báo danh sách đơn
struct list {
...
int main() {
	...
	for (int i = 1; i <= n; i++)
	{
		...
		ThemVaoCuoi(l, p); // Thêm vào đầu danh sách liên kết đơn
	}
